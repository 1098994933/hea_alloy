...
if __name__ == '__main__':
    ...
    for alg_name in alg_dict.keys():
        ...
        score = (-np.mean(
            cross_val_score(model, X, Y, groups=dataset["Group"], scoring='neg_mean_squared_error'))) ** 0.5

        y_predict = cross_val_predict(model, X, Y, groups=dataset["Group"])
    ...
    dataset[f'{Y_col}_predict'] = y_predict
